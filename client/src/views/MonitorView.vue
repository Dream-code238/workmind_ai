<template>
  <div class="monitor-view">
    <div class="metrics-grid">
      <MetricCard
        label="今日 API 调用"
        :value="s.overview?.apiCallsToday ?? 0"
        :sub="`总计 ${s.overview?.totalCallsToday ?? 0} 次`"
        color="blue"
      />
      <MetricCard
        label="缓存命中率"
        :value="s.overview?.cacheHitRate ?? '0%'"
        :sub="`命中 ${s.overview?.cacheHitsToday ?? 0} 次`"
        color="purple"
      />
      <MetricCard
        label="今日费用"
        :value="`¥${s.overview?.costCNYToday ?? 0}`"
        :sub="`预算 ¥${s.overview?.dailyBudget ?? 50}`"
        color="amber"
      />
      <MetricCard
        label="平均响应"
        :value="`${s.latency?.avg ?? 0}ms`"
        :sub="`P99: ${s.latency?.p99 ?? 0}ms`"
        color="green"
      />
    </div>
    ...
    <div class="budget-bar-wrap">
      <div class="budget-label">
        ...
        <span
          class="budget-pct"
          :class="{ warn: (s.overview?.budgetUsedPct ?? 0) >= 80 }"
          >{{ s.overview?.budgetUsedPct ?? 0 }}%</span
        >
      ...
      <div class="budget-bar">
        <div
          class="budget-fill"
          :style="{
            width: Math.min(s.overview?.budgetUsedPct ?? 0, 100) + '%',
          }"
          :class="{
            warn: (s.overview?.budgetUsedPct ?? 0) >= 80,
            danger: (s.overview?.budgetUsedPct ?? 0) >= 100,
          }"
        />
      </div>
      <div v-if="showBE" class="budget-edit">
        <input
          ...
          min="1"
        />
        <button class="btn btn-primary btn-xs" @click="updateBudget">
          保存
        </button>
        <button class="btn btn-ghost btn-xs" @click="showBE = false">
          取消
        </button>
      </div>
    </div>
        ...
        <div class="bar-chart">
          <div v-for="day in s.last7Days || []" :key="day.date" class="bar-col">
            <div class="bar-group">
              <div
                class="bar input-bar"
                :style="{ height: barH(day.inputT) + 'px' }"
                :title="`输入 ${day.inputT}`"
              />
              <div
                class="bar output-bar"
                :style="{ height: barH(day.outputT) + 'px' }"
                :title="`输出 ${day.outputT}`"
              />
            </div>
        ...
        <div v-else class="feature-list">
          <div v-for="f in s.byFeature" :key="f.feature" class="feature-row">
            ...
            <div class="feature-bar-wrap">
              <div
                class="feature-bar"
                :style="{ width: featureBarW(f.calls) + '%' }"
              />
            </div>
    ...
    <div class="table-card">
      ...
      <div class="table-wrap">
        <table class="call-table">
          ...
          <tbody>
            <tr v-if="!filteredCalls.length">
              <td colspan="7" class="empty-row">暂无记录，进行操作后刷新</td>
            </tr>
            <tr
              v-for="(c, i) in filteredCalls"
              :key="i"
              :class="{ 'from-cache': c.fromCache }"
            >
              <td class="time-cell">{{ fmtTime(c.time) }}</td>
              <td>
                <span class="feature-tag">{{ featureLabel(c.feature) }}</span>
              </td>
              <td>{{ c.inputT }}</td>
              <td>{{ c.outputT }}</td>
              ...
              <td>
                <span :class="c.fromCache ? 'cache-badge' : 'api-badge'">{{
                  c.fromCache ? "缓存" : "API"
                }}</span>
              </td>
            </tr>
...
const maxT = computed(() =>
  Math.max(...(s.value.last7Days || []).map((d) => d.inputT + d.outputT), 1),
);
const maxC = computed(() =>
  Math.max(...(s.value.byFeature || []).map((f) => f.calls), 1),
);
function barH(val) {
  return Math.max(2, Math.round((val / maxT.value) * 80));
}
function featureBarW(calls) {
  return Math.round((calls / maxC.value) * 100);
}
const latencyItems = computed(() => ({
  P50: s.value.latency?.p50 ?? 0,
  P90: s.value.latency?.p90 ?? 0,
  P99: s.value.latency?.p99 ?? 0,
  AVG: s.value.latency?.avg ?? 0,
}));
const filteredCalls = computed(() => {
  const c = s.value.recentCalls || [];
  return featureFilter.value
    ? c.filter((x) => x.feature === featureFilter.value)
    : c;
});
const featureOptions = computed(() =>
  [...new Set((s.value.recentCalls || []).map((c) => c.feature))].map((f) => ({
    feature: f,
