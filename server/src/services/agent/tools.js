...
export const searchTool = tool(
  async ({ query }) => {
    ...
    const mockResults = {
      ...
      React:
        "React 18.3 是最新稳定版，引入了并发渲染、useTransition、Suspense 改进。2024年主要关注点是 React Server Components 的稳定化。",
      Vite: "Vite 5.2 是目前最新版本，使用 Rollup 4 构建，冷启动速度提升 30%，支持 Lightning CSS。推荐用于新项目。",
      DeepSeek:
        "DeepSeek-V3 于2024年12月发布，是目前最强的开源 LLM 之一，性能接近 Claude 3.5 Sonnet，中文表现优秀，API 价格仅为 GPT-4o 的1/10。",
      TypeScript:
        "TypeScript 5.7 是最新版本，新增 noUncheckedSideEffectImports 选项，改进了声明文件的处理方式。",
      微前端:
        "qiankun 2.x 和 wujie 是国内最流行的微前端框架。wujie 基于 WebComponent + iframe，隔离性更好；qiankun 更成熟，社区更大。",
    };
    ...
    return key
      ? mockResults[key]
      : `关于"${query}"的搜索结果：该话题在技术社区有广泛讨论。建议查阅官方文档获取最准确的信息。`;
  },
  {
    name: "web_search",
    description:
    ...
    schema: z.object({
      query: z
        .string()
        .describe('搜索关键词，尽量精确，如"Vue3最新版本"而不是"前端框架"'),
    }),
  },
);
...
export const calculateTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      expression: z
        .string()
        .describe('数学表达式，如 "1500 + 800 * 0.8" 或 "(200 + 350) * 3"'),
    }),
  },
);
...
export const getDateTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      operation: z
        .enum(["today", "diff", "add_days"])
        .describe("today=获取今天日期, diff=计算日期差, add_days=日期加减"),
      date1: z
        .string()
        .optional()
        .nullable()
