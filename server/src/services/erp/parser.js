...
const ExpenseSchema = z.object({
  type: z
    .enum(["travel", "meal", "office", "training", "other"])
    .describe(
      "费用类型：travel=差旅, meal=餐饮, office=办公用品, training=培训, other=其他",
    ),
  items: z
    .array(
      z.object({
...
const LeaveSchema = z.object({
  type: z
    ...
    .describe(
      "假期类型：annual=年假, personal=事假, sick=病假, compensatory=调休, marriage=婚假, maternity=产假",
    ),
...
export function checkCompliance(expenseForm) {
  ...
  expenseForm.items.forEach((item) => {
    if (item.amount > rules.maxSingleItem) {
      alerts.push(
        `"${item.name}" ¥${item.amount} 超过单笔限额 ¥${rules.maxSingleItem}，需要额外说明`,
      );
    }
