...
async function runApproverTurn(
  roleId,
  formData,
  formType,
  conversationHistory,
  ...
  const questionResponse = await model.invoke([
    ...
    new HumanMessage(
      `请审核这份申请。如果有疑问，可以提问；如果信息充分，直接给出审批意见（批准/驳回）。`,
    ),
    ...conversationHistory,
  ]);
  ...
  if (hasQuestion && roleId !== "director") {
    ...
    const decisionResponse = await model.invoke([
      ...
      ...conversationHistory,
      new HumanMessage(
        `申请人已经回答了你的问题，现在请给出最终的审批意见：批准或驳回，并说明理由。`,
      ),
    ]);
...
export async function runApprovalFlow(formData, formType, onEvent) {
  ...
  const result = {
    ...
    approvedBy: allApproved
      ? approverIds.map((id) => APPROVAL_ROLES[id].name)
      : [],
