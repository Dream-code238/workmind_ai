...
const defaultTemplates = [
  {
    ...
    name: "前端助手",
    systemPrompt:
      "你是前端开发专家，精通 Vue3、React、TypeScript。回答简洁准确，必要时给代码示例。",
...
export function listTemplates() {
  return [...templateStore.values()].sort(
...
export async function scoreAbTest({ question, answerA, answerB }) {
  ...
  const [evalA, evalB] = await Promise.all([
    evalModel.invoke([
      {
        role: "system",
        content: "你是 AI 回答质量评估专家，客观评分，不偏袒任何一方。",
      },
  ...
  // 综合对比，判断胜者
  const compareModel = scoreModel.withStructuredOutput(
    z.object({
  ...
  const comparison = await compareModel.invoke([
    {
      role: "system",
      content: "比较两个回答，选出更好的那个。评分相差0.5分以内视为平局。",
    },
    {
      role: "user",
      content: `
