...
export function buildWeeklyReport() {
  ...
  async function extractHighlights(state) {
    ...
    const chain = ChatPromptTemplate.fromMessages([
      [
        "system",
        '你是写作助手，从工作要点中提炼亮点。输出3-5条，每条一行，用"• "开头，不超过30字。',
      ],
      ["human", "工作要点：\n{points}"],
    ])
      .pipe(model)
...
export function buildMeetingMinutes() {
  ...
  async function extractAttendees(state) {
    ...
    const chain = ChatPromptTemplate.fromMessages([
      [
        "system",
        "从会议记录中提取参会人员和主要议题。格式：参会人：xxx、xxx\n主要议题：xxx",
      ],
      ["human", "会议记录：\n{rawNotes}"],
    ])
      .pipe(model0)
...
export function buildEmailPolish() {
  ...
  async function analyzeIntent(state) {
    ...
    const chain = ChatPromptTemplate.fromMessages([
      ...
      .pipe(model0)
      .pipe(parser);
    const purpose = await chain.invoke({
      draft: state.draft,
      recipient: state.recipient || "对方",
    });
    return { purpose };
  }
...
如果没有明显问题，输出"整体质量良好，建议微调措辞使其更专业"。`,
  ...
  async function polishEmail(state) {
    ...
    const chain = ChatPromptTemplate.fromMessages([
      [
        "system",
        `你是专业邮件润色助手，根据分析结果优化邮件。${feedback}
...
export function buildPrdSkeleton() {
  ...
  async function extractFeatures(state) {
    ...
    const chain = ChatPromptTemplate.fromMessages([
      [
        "system",
        "从需求描述中提取核心功能点。按优先级排序，格式：P0/P1/P2 + 功能描述，每条一行。",
      ],
      ["human", "需求描述：\n{description}"],
    ])
      .pipe(model0)
...
如果描述中没有提及，写"待确认"。`,
  ...
  async function generatePrd(state) {
    ...
    const chain = ChatPromptTemplate.fromMessages([
      [
        "system",
        `你是产品经理助手，生成结构化 PRD 文档骨架。${feedback}
