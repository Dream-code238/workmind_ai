/**
 * @description 用量看板路由：API 调用统计、Token 消耗、缓存命中率、成本
 */
...
export function recordApiCall({
  ...
  const costUSD = (inputTokens / 1e6) * 0.27 + (outputTokens / 1e6) * 1.1;
  stats.calls.push({
...
monitorRouter.get("/stats", (req, res) => {
  ...
  const todayTotalCNY = todayCalls.reduce(
    (s, c) => s + (c.fromCache ? 0 : c.costCNY),
    0,
  );
  ...
  res.json({
    overview: {
      ...
      cacheHitRate: totalCalls
        ? `${((cacheHits / totalCalls) * 100).toFixed(1)}%`
        : "0%",
      ...
      budgetUsedPct: Math.min(
        100,
        parseFloat(((todayTotalCNY / stats.dailyBudget) * 100).toFixed(1)),
      ),
...
function getLast7DaysStats(calls) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({
      ...
      costCNY: parseFloat(
        dayCalls
          .reduce((s, c) => s + (c.fromCache ? 0 : c.costCNY), 0)
          .toFixed(4),
      ),
    });
  }
...
function getByFeature(calls) {
  const features = {};
  calls.forEach((c) => {
    if (!features[c.feature])
      features[c.feature] = { calls: 0, costCNY: 0, tokens: 0 };
