/**
 * @description 工作流路由：启动/查询/推进/获取结果
 */
...
import {
  WORKFLOW_BUILDERS,
  WORKFLOW_META,
} from "../services/workflow/workflows.js";
...
workflowRouter.post("/start/stream", rateLimiter, async (req, res) => {
  ...
  if (!workflowId || !WORKFLOW_BUILDERS[workflowId]) {
    return res
      .status(400)
      .json({ error: { message: `未知工作流：${workflowId}` } });
  }
        ...
        if (nodeInMeta) {
          ...
          if (output && typeof output === "object") {
            ...
            if (typeof firstVal === "string" && firstVal) {
              preview =
                firstVal.slice(0, 80) + (firstVal.length > 80 ? "..." : "");
            }
          }
...
workflowRouter.post("/resume/stream", rateLimiter, async (req, res) => {
  ...
  if (!wf) {
    return res
      .status(404)
      .json({ error: { message: "工作流不存在或已过期，请重新启动" } });
  }
