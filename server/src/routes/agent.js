/**
 * @description Agent 路由：流式执行任务，实时推送每一步状态
 */
...
agentRouter.post("/run", rateLimiter, securityCheck, async (req, res) => {
  const { task } = req.body;
  ...
  if (task.length > 2000) {
    return res
      .status(400)
      .json({ error: { message: "任务描述过长，请简洁描述" } });
  }
  ...
  try {
    ...
    logger.error("agent route error", {
      error: err.message,
      traceId: req.traceId,
    });
